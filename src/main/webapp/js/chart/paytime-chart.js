...
function reloadChart() {
	...
	$.post('chart/pay/search', $("#chartForm").serialize(), function(chartData) {
		if (chartData) {
			if (chartData.series && chartData.series != "") {
				showStackChartWithTotal(paytimeChart, paytimeOptions, '总购买次数: ', chartData.total, chartData.legend, 
						chartData.category, chartData.series, chartData.totals, register1);
				register1=true;
				showStackChartWithTotal(paymoneyChart, paymoneyOptions, '总购买金额: ', chartData.totalMoney, chartData.legend, 
