...
function reloadChart() {
	...
	$.post('chart/recharge/search', $("#chartForm").serialize(), function(chartData) {
		if (chartData) {
			if (chartData.series && chartData.series != "") {
				showStackChartWithTotal(paytimeChart, paytimeOptions, '总充值次数: ', chartData.total, chartData.legend, chartData.category, chartData.series, chartData.totals, register1);
				register1=true;
				showStackChartWithTotal(paymoneyChart, paymoneyOptions, '总充值金额: ', chartData.totalMoney, chartData.legend, chartData.category, chartData.moneySeries, chartData.totalMoneys, register2);
				register2=true;
			} 
		} else {
			showError("没有相关的统计数据!");
